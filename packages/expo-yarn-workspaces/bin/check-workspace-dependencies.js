...
function checkWorkspaceDependencies() {
  ...
  if (!workspacesInfo) {
    console.error(
      `Couldn't generate Yarn's workspace root information.`,
    )
    return
  }
  ...
  for (let workspaceName in workspacesInfo) {
    ...
    if (mismatchedWorkspaceDependencies.length) {
      ...
      console.warn(
        `   ${workspaceName} has ${
          mismatchedWorkspaceDependencies.length
        } ${singular ? "dependency" : "dependencies"} that ${
...
function _getWorkspacesInfo() {
  let result = child_process.spawnSync("yarn", [
    "--silent",
    "workspaces",
    "info",
  ])
...
function _countMismatchedWorkspaceDependencies(workspacesInfo) {
  ...
  for (let workspaceName in workspacesInfo) {
    count +=
      workspacesInfo[workspaceName].mismatchedWorkspaceDependencies
        .length
  }
  return count
}
